...
class ActorCNN(nn.Module):
    def __init__(self, action_dim, max_action, prior_dim, learning_rate):
    ...
    def forward(self, x, prior):
        ...
        x = self.dropout(x)
        x = self.lr(self.lin1(torch.cat([x, prior], 1)))
        ...
        # action[torch.isnan(action)] = self.max_action
...
class PriorCNN(nn.Module):
    def __init__(self, prior_dim, learning_rate, output_activation=None):
        ...
        self.prior_optimizer = torch.optim.Adam(self.parameters(), lr=learning_rate)
    ...
    def train_net(self, mini_batch):
        s, a, r, s_prime, done, prior = mini_batch
        pred_additional = self.forward(s)
        prior_loss = F.mse_loss(pred_additional, prior)
...
class SACRCRL(object):
    def __init__(self, state_dim, action_dim, max_action, prior_dim, lr_actor=1e-4, lr_critic=1e-3, lr_prior=1e-4, normalize_rew=True):
        super(SACRCRL, self).__init__()
        self.state_dim = state_dim
        self.prior_dim = prior_dim 
        self.state_dim = state_dim
        self.flat = False

        self.actor = ActorCNN(action_dim, max_action, prior_dim, lr_actor).to(device)

        self.critic_1 = CriticCNN(action_dim, lr_critic).to(device)
        self.critic_1_target = CriticCNN(action_dim, lr_critic).to(device)
        self.critic_2 = CriticCNN(action_dim, lr_critic).to(device)
        self.critic_2_target = CriticCNN(action_dim, lr_critic).to(device)

        self.critic_1_target.load_state_dict(self.critic_1.state_dict())
        self.critic_2_target.load_state_dict(self.critic_2.state_dict())

        self.prior_regressor = PriorCNN(prior_dim, lr_prior, output_activation='sigmoid').to(device)
    ...
    def predict(self, state):
        ...
        state = state.detach()
        self.actor.eval()
        self.prior_regressor.eval()
        with torch.no_grad():
            prior_prediction = self.prior_regressor(state)
            action = to_numpy(self.actor(state, prior_prediction)[0]).flatten()
        self.actor.train()
        self.prior_regressor.train()
        return action
    ...
    def train(self, replay_buffer, iterations, batch_size=64, discount=0.99, tau=0.001):
        ...
        for it in range(iterations):
            ...
            mini_batch = (state, action, reward, next_state, done, prior_t)

            # update prior network
            prior_loss = self.prior_regressor.train_net(mini_batch)
            ...
            # summarize losses
            total_prior_loss += to_numpy(prior_loss)
